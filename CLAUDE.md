...
Longcut is a resident process that owns its launch keys. Run without a subcommand, it binds the `keys_launch_global` and `keys_launch_app` configured under `x11: launcher:` on the X server through passive grabs and runs one navigation session per press. From the press on, the X server treats the keyboard as grabbed by longcut, so no key can be lost to another client; the session escalates that to a full keyboard grab and releases it when the session ends. The keys held to press the launch key are not modifiers of the keys that follow (`LaunchChord` in `longcut-x11`). A lock file in `$XDG_RUNTIME_DIR` keeps a second instance from starting. The X server releases every grab when the connection closes, so a crash cannot leave the keyboard grabbed.
...
- `Launcher` — wait for the user's request for a session; the returned `Session` token carries the mode and ends the session when dropped (the adapter owns the hotkeys, their configuration, and the keyboard grab of a session they begin)
- `Input` — read keys from the already-grabbed keyboard (the port only reads; the grab is taken by whoever begins the session)
...
- `longcut-x11` — x11rb client: hotkeys bound through passive grabs (`Hotkeys`), the session keyboard grab, the launch chord bookkeeping, key presses resolved through the X server's own keymap with xkbcommon, and active-window queries.
...
All composition happens in `longcut-application/src/main.rs`. Modules reference each other through `&'a` borrows, not `Arc`, and everything runs synchronously on one thread. The instance lock is taken first, so a second instance exits before setting anything up. The GUI is built next, and every later startup failure (a configuration error, a launch key that cannot be bound, a failed keyboard grab) is shown on screen for a few seconds since a session-spawned process has no terminal. Every configuration section is parsed before the launch keys are bound, so a configuration error never costs the user a grab. The launch keys are bound last and handed to `CoreService::run_forever`.
